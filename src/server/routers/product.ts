...
import { z } from "zod";
import { eq, and, like, sql, isNotNull, or, inArray } from "drizzle-orm";
...
export const productRouter = createTRPCRouter({
    ...
    .query(async ({ ctx, input }) => {
      return ctx.db.transaction(async (trx) => {
        const result = await trx
          ...
          .from(products)
          .where(and(eq(products.id, input)))
          .limit(1);
      ...
      return ctx.db.transaction(async (trx) => {
        ...
        const whereClause = and(
          filters.keyword // Filter: Keyword
            ...
            : undefined,
          filters.category && filters.category?.length > 0 // Filter: Category
            ? inArray(products.category, filters.category)
            : undefined,
          filters.type && filters.type?.length > 0 // Filter: Type
            ? inArray(products.type, filters.type)
            : undefined
        );
  ...
  getCatagories: protectedProcedure
    .output(z.array(Product.schemas.baseOutput.shape.category))
    .query(async ({ ctx }) => {
      return ctx.db.transaction(async (trx) => {
  ...
  getUnits: protectedProcedure
    .output(z.array(Product.schemas.baseOutput.shape.unit))
    .query(async ({ ctx }) => {
      return ctx.db.transaction(async (trx) => {
